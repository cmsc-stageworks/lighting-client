...
| F-MAP-06 | P0 | **Mapping** = name, enabled flag, trigger (F-THOR-07/08 or F-MQTT-03 or a UI event), one or more actions (`activateScene`, `releaseScene`, `releaseLayer`, `blackout on/off`, `setLevel`, `holdLevel`, `publishMqtt`, `thoriumMutation`, `setMappingGroup`), optional simulator restriction, optional mapping groups (F-MAP-13), optional debounce ms, notes. |
| F-MAP-10 | P0 | **Channel level** (`setLevel` action): hold one or more channels at a 0–255 value taken from a number on the event, re-scaled through a configurable input range, output range and response curve. Addressing is relative (offsets from the simulator's base address) or absolute. Repeat events update the same channels in place and crossfade the *value*; the fade time can be fixed or taken from the event, which is how a follow of Thorium's lighting intensity reproduces the FD's transition duration. Allowed in Reduced Effects, held back in Locked. Optional **no-signal level**: a 0–255 value held on the Base layer whenever the level has no live signal — from app start (before Thorium answers), while unassigned or disconnected, and after releases — so crew are never left in full darkness by default. Thorium's starting intensity of 0 (the first reading after a connect) counts as no signal unless turned off; a later FD change to 0, and Blackout, still go fully dark. |
| F-MAP-12 | P1 | **Timed channel hold** (`holdLevel` action): the mirror of F-MAP-10 — the *duration* comes from a number on the event (in ms or seconds, with a fallback when the event carries none) and the 0–255 value is set in the action. Holds one or more channels at that value, then releases them; the hold starts once the fade in finishes and re-firing the same mapping re-arms it rather than stacking a second instance. Optionally a **second stage**: instead of releasing when the hold ends, the channels move to a configurable fallback value and hold *that* for its own time (or until staff release it) before letting go — trigger → value for the event's duration → fallback value → release, all from one event. Addressing, layer and fade work as in F-MAP-10. Held back in both Reduced Effects and Locked — a short hold is a flash. |
| F-MAP-13 | P1 | **Mapping groups**: named, coloured, switchable sets of mappings in a profile. A mapping can belong to any number of groups; one in no group fires in every group. Exactly one group is active (the first until changed); only its mappings plus ungrouped ones fire. Staff switch from a control in the status bar and a tray submenu; a mapping can switch with a `setMappingGroup` action (e.g. from a Thorium macro). Switching releases looks from mappings that dropped out and re-applies the current alert level so the new group's look shows immediately. The choice persists across restarts with no daily reset. Use: one alert-5 look in one group and a different one after staff change the setting. |
