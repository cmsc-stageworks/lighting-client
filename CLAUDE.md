...
- **Mapping groups** (`shared/mappingGroups.ts`, ERD §5.1b): `profile.mappingGroups` + `mapping.groupIds` (empty = every group). One group is active; `RulesEngine.setActiveGroup` filters, `Services.setMappingGroup` (status bar, tray, `setMappingGroup` action) releases dropped-out looks and re-asserts alert levels. Active group is persisted in `userData/mapping-group.json` (no daily reset), not config.json.
- **No-signal floors** (ERD §6.3b): a `setLevel` with `idle` gets a Base-layer instance (`origin.floor`) applied by `ActionRunner.applyFloors()` at startup, on config change and group switch; it survives release-all/scope releases, and an `initial` zero reading releases the live level so the floor shows. Thorium creates sims at intensity 0, which is why this exists.
